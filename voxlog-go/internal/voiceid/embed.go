// Package voiceid turns a stretch of speech into a fingerprint, and
// fingerprints into people.
//
...
// is not enough speech to fingerprint. Normalised here, once, so every
// comparison downstream is a plain dot product and no caller has to remember.
//
...
func (e *Embedder) Compute(samples []float32) []float32 {
	...
	return Normalize(e.impl.Compute(stream))
}
